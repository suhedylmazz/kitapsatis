...
// Kullanıcının bu ürüne verdiği oy (varsa)
$userVote = 0;
if (isLoggedIn()) {
    $user_id = $_SESSION['user_id'];
    $userSql = "SELECT rating FROM product_ratings WHERE product_id=? AND user_id=?";
    $stmt3 = $mysqli->prepare($userSql);
    $stmt3->bind_param("ii", $product_id, $user_id);
    $stmt3->execute();
    $userRes = $stmt3->get_result();
...
<head>
  ...
  <style>
    ...
    .discount-price {
      background-color:rgb(241, 187, 23);
      padding: 8px 10px;
      font-weight: 900;
      color: #000;
      border-radius: 4px;
          ...
          <div class="col-md-8">
            ...
            <ul class="list-group mb-3">
              <li class="list-group-item">
                <strong>Yayınevi:</strong> <?php echo htmlspecialchars($product['publisher'] ?? ''); ?>
              </li>
              <li class="list-group-item">
                <strong>Dil:</strong> <?php echo htmlspecialchars($product['language'] ?? ''); ?>
              </li>
              <li class="list-group-item">
                <strong>Baskı Yılı:</strong> <?php echo htmlspecialchars($product['publication_year'] ?? ''); ?>
              </li>
              <li class="list-group-item">
                <strong>Sayfa Sayısı:</strong> <?php echo htmlspecialchars($product['page_count'] ?? ''); ?>
              </li>
              <li class="list-group-item">
                <strong>Fiyat:</strong> <?php echo number_format($originalPrice, 2, ',', '.'); ?> TL
              </li>
            </ul>
...
<script src="https://cdn.jsdelivr.net/gh/diorcao/elevatezoom/jquery.elevatezoom.js"></script>
...
$(document).ready(function(){
  ...
  // Yıldızlı oylama (opsiyonel)
  document.querySelectorAll('.star').forEach(function(star) {
    star.addEventListener('click', function(){
      let ratingValue = this.getAttribute('data-value');
      let productId   = <?php echo (int)$product_id; ?>;
      fetch('rate_product.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'rating=' + ratingValue + '&product_id=' + productId
      })
      .then(response => response.text())
      .then(data => {
        const ratingResult = document.getElementById('rating-result');
        if (ratingResult) {
          ratingResult.textContent = data;
        }
      ...
      .catch(err => {
        console.error(err);
        if (document.getElementById('rating-result')) {
          document.getElementById('rating-result').textContent = 'Bir hata oluştu.';
        }
      });
    });
  });
